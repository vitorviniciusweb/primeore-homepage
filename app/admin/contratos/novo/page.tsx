...
export default function NovoContratoPage() {
  ...
  function updateCliente(id: string, field: keyof ClienteParte, value: string) {
    setClientes(prev =>
  ...
  function updateSecao(id: string, field: keyof Secao, value: string) {
    setSecoes(prev =>
  ...
  async function handleGerarContrato() {
    ...
    try {
      ...
      if (!saveRes.ok) throw new Error('Falha ao salvar contrato.')

      // Generate and download PDF
      const pdfRes = await fetch(`/api/contracts/pdf/${novoContrato.id}`)
      if (pdfRes.ok) {
        const blob = await pdfRes.blob()
        const url = URL.createObjectURL(blob)
        const a = document.createElement('a')
        a.href = url
        a.download = `contrato-${novoContrato.id}.pdf`
        a.click()
